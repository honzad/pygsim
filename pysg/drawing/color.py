...
def validate_enumerations(d: Dict[str, Union[str, int]]) -> None:
	for data in d.values():
		...
		match data_type:
			...
			case _:
				raise ValueError("Invalid color type supplied, has to be either int or string")
	pass

def generate_color_palette(n: int = 5) -> List[Tuple[int, int, int]]:
	HSV_tuples = [(x*1.0/n, 0.5, 0.5) for x in range(n)]
	RGB_tuples = map(lambda x: colorsys.hsv_to_rgb(*x), HSV_tuples)
	return list(map(lambda x: tuple(clamp(math.ceil(255.0 * elem), 0, 255) for elem in x),RGB_tuples))

def color_enumerations_to_colors(d: Dict[str, Union[str, int]]) -> Dict[str, pygame.Color]:
